...
export const MessageBubble: React.FC<MessageBubbleProps> = ({ message, personaAvatarSeed, personaName }) => {
  ...
  return (
    ...
    >
      ...
      {isUser && (
        <div className="w-8 h-8 rounded-full bg-blue-600 flex items-center justify-center font-mono font-bold text-white text-xs shrink-0 shadow-xs">
          U
        </div>
      )}
      ...
      <div className="flex flex-col gap-1.5">
        ...
        {/* Sender and Time */}
        <div className={`flex items-center gap-2 font-mono text-[10px] text-slate-400 ${isUser ? 'justify-end' : 'justify-start'}`}>
        ...
        <div className={`relative px-4 py-3 rounded-2xl border transition-all duration-200 shadow-xs ${
          ...
          {message.translation && (
            <div className={`mt-2 pt-2 border-t border-dashed transition-all duration-200 ${
              ...
              <p className={`font-sans text-[13px] italic ${isUser ? 'text-blue-100' : 'text-slate-500'}`}>
                {message.translation}
              </p>
            </div>
          )}
